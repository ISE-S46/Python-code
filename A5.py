    ...
    def check3(f,g,h,id,x,nt1):
        ...
        if str(id) in str(cond):
            if nt1<4:
                ...
                print('===========================================================================')   
        return nt1
    def check2(f,g,h,id,x,nt,n3):
        ncond = 0
        if h < (float(f)/float(g)):
            cond = (f/g) + h
            ncond = 1
        elif h >= (f/g):
            cond = ((f-1)*h)/g
        ...
        if str(id) in str(cond):
            if nt<4:
                ...
                print(form.format(n3,x,f,g,h))
                print(form2.format(cond))
        return nt
    def check(fx,gx,hx,id,n2):
...
for x in range (a1,a2,1):
    ...
    gx = A.g(fx[0])
    hx = A.h(gx[0])
    n2 = A.check(fx[0],gx[0],hx[0],id,n2)
    nt = A.check2(fx[0],gx[0],hx[0],id,x,nt,n3)
if len(z) == 0:
...
for x in range (a1,a2,1):
    ...
    gx = A.g(fx[0])
    hx = A.h(gx[0])
    nt1 = A.check3(fx,gx,hx,id,x,nt1)
if len(z) == 0 :
